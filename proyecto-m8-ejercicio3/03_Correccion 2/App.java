...
public class App {
				...
				switch (opcionMenuElegida) {
					...
					case 2: // ELIMINAR CLIENTE ---------------------------------------------------------------
						...
						if (posicionClienteEnArray>=0) {
							if (listaClientes.get(posicionClienteEnArray).tieneCuentas()) {
					...
					case 3: // CREAR NUEVA CUENTA A UN CLIENTE -----------------------------------------------
						...
						if (posicionClienteEnArray>=0) {
							...
							abrirCuenta(numeroCuentaGenerada, listaClientes.get(posicionClienteEnArray));
	...
	static void abrirCuenta(int numCuenta, Cliente titular) {
		Cuenta nuevaCuenta = new Cuenta(numCuenta, titular);
		titular.incluirCuentaEnArray(nuevaCuenta);
	}
	...
	static void eliminarCuenta(int indiceCuentaOperacion, ArrayList<Cliente> listaClientes, int posicionClienteEnArray) {
		if ((listaClientes.get(posicionClienteEnArray).getListaCuentasCliente().get(indiceCuentaOperacion).tieneUnSaldoDe(0))>0) {
			...
			listaClientes.get(posicionClienteEnArray).eliminarCuentaDeArray(indiceCuentaOperacion);
